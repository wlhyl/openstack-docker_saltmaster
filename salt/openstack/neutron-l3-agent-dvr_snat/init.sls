...
{{ pillar['docker']['registry'] }}/lzh/neutron-l3-agent:
  docker.pulled:
    - name: {{ pillar['docker']['registry'] }}/lzh/neutron-l3-agent-dvr_snat
    - tag: {{ openstack_version }}
    - insecure_registry: True

neutron-l3-agent_docker:
  docker.running:
    - name: neutron-l3-agent-dvr_snat
    - image: {{ pillar['docker']['registry'] }}/lzh/neutron-l3-agent-dvr_snat:{{ openstack_version }}
    - environment:
